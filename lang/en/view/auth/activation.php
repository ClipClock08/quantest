...
if(isset($_GET['token']) && !empty($_GET['token'])){
    ...
    exit("<p><strong>Ошибка!</strong> Missing verification code.</p>");
...
if(isset($_GET['email']) && !empty($_GET['email'])){
    ...
    exit("<p><strong>Ошибка!</strong> Missing email address.</p>");
...
if(!$query_delete_users){
    ...
    exit("<p><strong>Ошибка!</strong> Failed to delete expired account. Error code: ".$mysqli->errno."</p>");
...
if(!$query_delete_confirm_users){
    ...
    exit("<p><strong>Ошибка!</strong> Failed to delete expired account(confirm). Error code: ".$mysqli->errno."</p>");
...
$query_select_user = $mysqli->query("SELECT DISTINCT `token` FROM `confirm_users` WHERE `email` = '".$email."'");
    ...
    if($query_select_user->num_rows == 1){
        ...
        if($token == $row['token']){
            ...
            if(!$query_update_user){
                ...
                exit("<p><strong>Ошибка!</strong> Failed to update user status. Error code: ".$mysqli->errno."</p>");
                ...
                if(!$query_delete){
                    ...
                    exit("<p><strong>Ошибка!</strong> Failed to delete user data from temporary table. Error code: ".$mysqli->errno."</p>");
                    ...
                    require_once("../header_q.php");
                        ...
                        echo '<h1 class="success_message text_center">Mail successfully verified!</h1>';
                        echo '<p class="text_center">Now you can <a href="form_auth.php">login</a> to your account..</p>';
            ...
            exit("<p><strong>Ошибка!</strong> Invalid verification code.</p>");
        }

    }else{ //if($query_select_user->num_rows == 1)
        exit("<p><strong>Ошибка!</strong> This user is not registered </p>");
    }

}else{ //if(($row = $query_select_user->fetch_assoc()) != false)
    //Иначе, если есть ошибки в запросе к БД
    exit("<p><strong>Ошибка!</strong> Failure when selecting a user from the database. </p>");
}
