...
if (isset($_POST["upload"]) && !empty($_POST["upload"])) {
    ...
    $path = 'https://quantest.eu/lang/de/uploads/'.$file_name;
    echo $path;
...
elseif (isset($_POST["add_candidates"]) && !empty($_POST["add_candidates"])) {
    ...
    for($i = 0; $i < $n; $i++) {
        ...
        $result_query_insert = $mysqli->query("INSERT INTO `candidates` (user_id, family_name, first_name,born_date, gender, cv_status, language) VALUES ($user_id,'" . $family_name[$i] . "','" . $first_name[$i] . "','".$born_date[$i]."','" . $gender[$i] . "', '$cv_status', 'Deutsch')");
