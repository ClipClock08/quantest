    ...
    if (isset($_POST["nomClient"])) {
       ...
       $first_name = $_POST['nomClient'];
        ...
        if (!empty( $first_name)) {
            ...
            header("Location: " . $address_site . "alexWorks/admin/admin_recrutement.php");
...
elseif(isset($_POST["saveResult"]) && !empty($_POST["saveResult"])){
    ...
    if (isset($_SESSION["clientName"]) && !empty($_SESSION["clientName"])) {
     ...
     if (isset($_POST["selectCandidat"]) && !empty($_POST["selectCandidat"])) {

        //Обрезаем пробелы с начала и с конца строки
        $test_type = trim($_POST["selectCandidat"]);
    ...
    if (!$result_query_insert) {
        ...
        exit();
            ...
            header("HTTP/1.1 301 Moved Permanently");
            header("Location: " . $address_site . "../alexWorks/admin/test_result.php");
            exit();
    }
